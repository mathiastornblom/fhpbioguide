...
func createPresaleForm(service reportform.UseCase) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if authToken["Authorization"] != viper.GetString("report.Bearer") {
	...
	return func(c *fiber.Ctx) error {
		...
		if form.Type == 0 {
			...
			for id, event := range form.Events {
				...
				if standardTicket == "0" && freeTicket == "0" && kidsTicket == "0" && playsTicket == "0" && scenTicket == "0" && konstTicket == "0" && otherTicket == "0" && other2Ticket == "0" {
